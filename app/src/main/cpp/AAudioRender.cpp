...
int AAudioRender::flush() { // 用于清空缓冲区
...
int AAudioRender::pause(bool p) { // 暂停播放
    if (p == paused) {
        return 0;
    }
    if (p) {
...
//void AAudioRender::setCallback(AAudioCallback cb, PacketQueue* packetQueue) {
//    this->callback = cb;
//    this->user_data = packetQueue;
//}
